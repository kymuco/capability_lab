"""Strict deterministic serialization for PR12.4 claim materialization receipts."""
...
def external_evidence_interpretation_claim_materialization_receipt_to_dict(
    value: ExternalEvidenceInterpretationClaimMaterializationReceipt,
) -> dict:
    _strict_materialization_receipt(value)
    return {
        ...
        "materialized_at": _format_time(value.materialized_at),
    }


def external_evidence_interpretation_claim_materialization_receipt_from_dict(
    payload: object,
) -> ExternalEvidenceInterpretationClaimMaterializationReceipt:
    fields = {
        "schema_version",
    ...
    obj = _obj(payload, fields, "claim materialization receipt")
    if type(obj["schema_version"]) is not int or obj["schema_version"] != _SCHEMA_VERSION:
        _fail("claim materialization receipt schema_version must be exact integer 1")
    try:
        result = ExternalEvidenceInterpretationClaimMaterializationReceipt(
            ...
            materialized_at=_parse_time(obj["materialized_at"], "materialized_at"),
        )
    except (TypeError, ValueError) as exc:
        if isinstance(exc, InvalidExternalEvidenceInterpretation):
            raise
        raise InvalidExternalEvidenceInterpretation(
            f"invalid claim materialization receipt: {exc}"
        ) from exc
    return _strict_materialization_receipt(result)


def external_evidence_interpretation_claim_materialization_receipt_to_json(
    value: ExternalEvidenceInterpretationClaimMaterializationReceipt,
) -> str:
    return _dumps(
        external_evidence_interpretation_claim_materialization_receipt_to_dict(value)
    )


def external_evidence_interpretation_claim_materialization_receipt_from_json(
    payload: object,
) -> ExternalEvidenceInterpretationClaimMaterializationReceipt:
